defmodule LiveStudy.Boats do
  @moduledoc """
  The Boats context.
  """

  alias LiveStudy.Boats.Boat
  ...
  defp filter_by_type(query, %{type: type}), do: where(query, type: ^type)

  defp filter_by_prices(query, %{prices: [""]}), do: query
  defp filter_by_prices(query, %{prices: prices}), do: where(query, [boat], boat.price in ^prices)
