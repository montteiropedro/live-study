defmodule LiveStudy.DesksTest do
  use LiveStudy.DataCase
  ...
  describe "desks" do
    ...
    import LiveStudy.DesksFixtures
    ...
    test "create_desk/1 with valid data creates a desk" do
      valid_attrs = %{name: "some name", photo_locations: "some photo_locations"}

      assert {:ok, %Desk{} = desk} = Desks.create_desk(valid_attrs)
      assert desk.name == "some name"
      assert desk.photo_locations == "some photo_locations"
    end
    ...
    test "update_desk/2 with valid data updates the desk" do
      desk = desk_fixture()
      update_attrs = %{name: "some updated name", photo_locations: "some updated photo_locations"}

      assert {:ok, %Desk{} = desk} = Desks.update_desk(desk, update_attrs)
      assert desk.name == "some updated name"
      assert desk.photo_locations == "some updated photo_locations"
    end
